...
import { CreateRefillSuccess, DeleteRefillSuccess, RefreshRefillSuccess, RefreshUserSuccess } from '@/redux/slice/user';
...
function UserRefill(props) {
    ...
    //DataUser
    const Refills = useSelector(UserSelector.Payments.Refills);
    ...
    const hadleCreateRefill = async () => {
        if (AccessToken && User) {
            await UserPaymentsApi.Money.CreateRefill(AccessToken, axiosJwt, User?.id, amount, idBankUser, idBankPublic, photo, dispatch, CreateRefillSuccess);
    ...
    const handleDeletePayment = async (payment) => {
        await UserPaymentsApi.Money.Delete(AccessToken, axiosJwt, payment.id);
        await UserPaymentsApi.Money.Refresh(AccessToken, axiosJwt, User?.id, dispatch, RefreshRefillSuccess, RefreshUserSuccess);
    };
    ...
    useEffect(() => {
        ...
        const listRender = Refills.filter(item => item.status === "Pending");
        const filter = listRender.slice(offset, (offset + limit));
