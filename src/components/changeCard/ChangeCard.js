...
function UserChangeCard(props) {
    ...
    const handleChangeCard = async () => {
        if (code.length > 10 && serial.length > 10) {
            ...
            await ApiCardsPublic.PostCards(telco, idValue, code, serial, User?.id, accessToken, dispatch, idToast, axiosJwt);
            await ApiUsers.Data.LoadingDataUser(User?.id, dispatch);
            setCode("");
            setSerial("");
        } else {
