...
function ModalViewsUser({show, setShow, user}) {
    ...
    const ChangeCardsRender = ChangeCards?.filter(
        item => item.idUser === user.id && item.status !== "Error"
    )
    const BuyCardsRender = BuyCards?.filter(
        item => item.idUser === user.id && item.status !== "Error"
    )
    ...
    useEffect(() => {
        ...
        for (let index = 0; index < ChangeCardsRender.length; index++) {
            ...
            b +=
                Number(ChangeCardsRender[index].Price?.Value?.name) -
                (Number(ChangeCardsRender[index].Price?.Value?.name) *
                    Number(ChangeCardsRender[index].Price?.feesChange)) /
                    100
            setTotalChange({
                change: a,
                receive: b,
        ...
        for (let index = 0; index < BuyCardsRender.length; index++) {
            ...
            d +=
                Number(BuyCardsRender[index].Price?.Value?.name) -
                (Number(BuyCardsRender[index].Price?.Value?.name) *
                    Number(BuyCardsRender[index].Price?.feesBuy)) /
                    100
            setTotalBuy({
                buy: c,
                receive: d,
                ...
                <Modal.Body className="txt_dark_100 bgr_linear">
                    <div className="change_cards layot_wrapter">
                        ...
                        <div className="total_price d-flex justify-content-between align-items-center">
                            <p>
                                {" "}
                                Total Change: {formatMoney(totalChange.change)}
                            </p>
                            <p>
                                {" "}
                                Total Receive:{" "}
                                {formatMoney(totalChange.receive)}
                            </p>
                            <p>
                                {" "}
                                FreeMoney:{" "}
                                {formatMoney(
                                    totalChange.change - totalChange.receive
                                )}
                            </p>
                        </div>
