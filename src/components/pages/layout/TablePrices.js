    ...
    return (
        <div className="table_prices bgr_dark mt-3 animate__animated  animate__fadeInUp">
            <div className="hearder_hag">
                <h1>
                    Bảng Phí Đổi Thẻ Ngày:{" "}
                    <span className="text-danger">{handleRenderToday()}</span>
                </h1>
            </div>
            <div className="price_content">
                <div className="prices_hearder">
                    {listCardsChange?.map((card, index) => {
                        return (
                            <Button
                                onClick={() => setCardRender(card.telco)}
                                variant={
                            ...
                            <tr>
                                ...
                                {PriceRender?.map((price, index) => {
                                    return (
                                        <th
                                            key={index}
                                            className="txt_center txt_white"
                                        >
                                            {formatMoney(price.Value.name)}
                                        </th>
                                    )
                                })}
                            </tr>
