        ...
        try {
            ...
            if (payment) {
                if (payment.status === "Pending") {
                    ...
                    if (user) {
                        ...
                        await payment.save().then(async () => {
                            if (status === "Success") {
                                ...
                                return res
                                    .status(200)
                                    .json({mess: "Xác nhận thành công!"})
                            } else if (status === "Error") {
                                return res
                                    .status(200)
                                    .json({mess: "Hủy lệnh thành công!"})
                            }
                        })
                    } else {
                    ...
                    return res
                        .status(400)
                        .json({error: "Lệnh đã được xác nhận trước đó!"})
                }
            } else {
                return res
                    .status(404)
                    .json({error: "Không tìm thấy lệnh trên hệ thống!"})
            }
        } catch (error) {
            ...
            try {
                ...
                if (withdraw) {
                    if (withdraw.status === "Pending") {
                        ...
                        return res
                            .status(400)
                            .json({error: "Lệnh rút tiền đã được xác thực!"})
                    }
                } else {
                    return res
                        .status(404)
                        .json({error: "Lệnh rút tiền không tồn tại!"})
                }
            } catch (error) {
                return res.status(500).json(error)
            }
