import { Cards, Imgs, Prices, RefreshTokens, Users, Values } from "data/db/models";
                            ...
                            if (bcryptjs.compareSync(pass2, admin.pass2)) {
                                if (keyAdmin === process.env.NEXT_PUBLIC_KEY_ADMIN) {
                                    ...
                                    if (!created) {
                                        ...
                                        admin.pass = null;
                                        admin.pass2 = null;
                                        const str = admin.wallet_number.split(" ")[1];
                                    ...
                                    return res.status(404).json({ error: "Key admin không chính xác!" });
                                }

                            } else {
                                return res.status(404).json({ error: "Mật khẩu cấp 1 không chính xác!" });
                            }
                        } else {
                            return res.status(404).json({ error: "Mật khẩu cấp 1 không chính xác!" });
                        }
