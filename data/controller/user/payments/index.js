                    ...
                    if (bankUser) {
                        ...
                        if (bankPublic) {
                            if (req.file) {
                                ...
                                const [img, createdImg] =
                                    await Imgs.findOrCreate({
                                        where: {
                                            path: pathImage,
                                        },
                                    })
                                if (!createdImg) {
                                    ...
                                    await img.save().then(async newImg => {
                                        ...
                                        if (!created) {
                                            return res.status(400).json({
                                                error: "Lệnh đã được gửi lên vui lòng không spam!",
                                            })
                                        } else {
                                            refill.set({
                                                idUser: user.id,
                                ...
                                return res.status(400).json({
                                    error: "Hình ảnh xác minh không được để trống!",
                                })
                            }
                        } else {
                            return res.status(400).json({
                                error: "Ngân hàng nhận tiền không chính xác!",
                            })
                        }
                    }
                } else {
            ...
            try {
                ...
                if (user) {
                    const list = await Payments.findAll({
                        where: {
                            [Op.and]: [{idUser: user.id}, {command: "refill"}],
                        },
                        include: [
                ...
                if (user) {
                    ...
                    if (surplus > 0) {
                        ...
                        if (bankUser) {
                            ...
                            if (!created) {
                                return res.status(400).json({
                                    error: "Lệnh đã được gửi lên hệ thống! Vui lòng không spam.",
                                })
                            } else {
                                payment.set({
                                    idUser: user.id,
                            ...
                            return res.status(404).json({
                                error: "Ngân hàng khách hàng không tồn tại!",
                            })
                        }
                    } else {
